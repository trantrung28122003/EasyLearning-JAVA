...
import { GET_ALL_COURSE, GET_COURSES_MOST_REGISTERED, GET_COURSES_WITH_DISCOUNT, GET_FEEDBACKS_WITH_FIVE_RATING, GET_TOP_FOUR_MOST_CATEGORY } from "../../../constants/API";
...
const Home: React.FC = () => {
  ...
  const [coursesWithDiscount, setCoursesWithDisCount] = useState<Course[]>([]);
  ...
  const doGetCoursesWithDiscount = () => {
    DoCallAPIWithOutToken(GET_COURSES_WITH_DISCOUNT, "get").then((res) => {
      if (res.status === HTTP_OK) {
        const data: ApplicationResponse<Course[]> = res.data;
        setCoursesWithDisCount(data.result);
      }
    });
  };
  ...
  const doGetFeedBackWithFiveRating= () => {
    DoCallAPIWithOutToken(GET_FEEDBACKS_WITH_FIVE_RATING, "get").then(response =>{
      if(response.status=== HTTP_OK){
    ...
    <ClientShared>
                        ...
                        <div className="container">
                            <div className="row justify-content-start">
                                <div className="col-sm-10 col-lg-8">
                                    ...
                                    <a href="/about" className="btn btn-primary py-md-3 px-md-5 me-3 animated slideInLeft">Xem Thêm</a>
                                    <a href="/courses/search" className="btn btn-light py-md-3 px-md-5 animated slideInRight">Tham Gia Ngay</a>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div className="container-xxl py-5">
          <div className="container">
            ...
            <div className="row g-4 justify-content-center">
              {coursesWithDiscount.map((course, index: number) => {
                return <LongCard course={course} key={index} />;
              })}
            </div>
          </div>
        </div> 
