...
const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({
  ...
  const [isRedirecting, setIsRedirecting] = useState(false); // Kiểm tra xem người dùng có được chuyển hướng hay không
  ...
  const resetForm = () => {
    ...
    setConfirmPassword("");
    setErrorMessage("");
    setCountdown(5); // Reset thời gian đếm ngược khi reset form
    setIsRedirecting(false); // Reset trạng thái redirect
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.result === "Thay đổi mật khẩu thành công") {
  ...
  // Bắt đầu đếm ngược sau khi đổi mật khẩu thành công
  const startCountdown = () => {
    setIsRedirecting(true); // Người dùng sẽ được chuyển hướng
    const interval = setInterval(() => {
      setCountdown((prev) => {
        if (prev === 1) {
          clearInterval(interval);
          navigate("/login"); // Chuyển hướng đến trang login khi hết thời gian
          return 0;
        }
        return prev - 1;
      });
  ...
  // Đóng modal khi người dùng click ngoài modal
  const handleOutsideClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (e.target === e.currentTarget) {
      resetForm(); // Gọi hàm reset khi đóng modal
      onClose();
    }
  };
